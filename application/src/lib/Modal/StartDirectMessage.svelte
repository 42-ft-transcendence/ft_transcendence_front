    ...
    <form class="modal-form {cForm}">
        <label class="label">
            ...
            <div class="input-group input-group-divider grid-cols-[auto_1fr_auto]">
                ...
                <button class="variant-filled-secondary hover:brightness-125 active:scale-[95%] active:brightness-90" on:click={searchUser} >Search</button>
            </div>
        </label>
        {#if users === undefined}
                    ...
                    {#each users as {avatar, name}, i}
                        ...
                        <li>
                            <div class="group w-full grid grid-cols-[1fr_auto] h-12 p-2 rounded-md hover:bg-surface-400">
                                ...
                                <button type="button" class="btn btn-sm variant-filled hidden group-hover:block" on:click={startDM}>대화 시작</button>
                            </div>
                        </li>
                    {/each}
                    </ul>
