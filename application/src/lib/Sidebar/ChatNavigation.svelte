            ...
            <nav class="list-nav pr-2 pt-2">
                ...
                {#each data.chat[0].list as {href, type, name} }
                    <li on:contextmenu|preventDefault={handleRightClickedChannel}>
                        <a {href} class="{classesActive(href)}"><i class={channelIcon[type]} aria-hidden="true" />&nbsp;{name}</a>
                    </li>
                {/each}
                </ul>
                <button class="w-full mt-1" on:click|stopPropagation={handleAddChannel}><i class="fa fa-plus-square" aria-hidden="true"/>&nbsp;add channels</button>
            </nav>
        </svelte:fragment>
    </TreeViewItem>
    <TreeViewItem open={true}>
        ...
        <svelte:fragment slot="children">
            <nav class="list-nav pr-2 pt-2">
                ...
                <button class="w-full mt-1" on:click={addCoworkers}><i class="fa fa-plus-square" aria-hidden="true"/>&nbsp;add coworkers</button>
            </nav>
        </svelte:fragment>
    </TreeViewItem>
</TreeView>
{#if contextmenuComponent !== undefined}
<svelte:component this={contextmenuComponent} {pointerEvent} />
{/if}
